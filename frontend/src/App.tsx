...
export default function App() {
  ...
  const trackedJobIdRef = useRef<string | null>(trackedJobId);
  ...
  const loadResults = useCallback(async (runId: string, jobId: string) => {
    ...
    try {
      ...
      if (
        ...
        trackedJobIdRef.current !== jobId || schedule.job_id !== jobId || report.job_id !== jobId ||
  ...
  useEffect(() => {
    if (!polledJob) return;
    if (polledJob.state !== "COMPLETED" && loadedJobId != null) {
      resultRequestRef.current += 1;
      setScheduleData(null);
      setReportData(null);
      setLoadedJobId(null);
  ...
  const resultReady =
    ...
    scheduleData.job_id === trackedJobId &&
    reportData.job_id === trackedJobId &&
    polledJob?.state === "COMPLETED" &&
  ...
  return (
    <div className="app">
      ...
      <main className="workflow-content">
        <section id="workflow-panel-calendar" role="tabpanel" aria-labelledby="workflow-tab-calendar"
          tabIndex={-1} hidden={activeStage !== "calendar"} className="workflow-panel">
          {resultReady && scheduleData && selectedRun && activeStage === "calendar" ?
            <PossessionCalendar key={scheduleData.job_id} runId={selectedRun.id} jobId={scheduleData.job_id} /> : null}
        </section>
        <section

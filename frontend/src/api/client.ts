...
export function getCalendar(run: string, job: string): Promise<PossessionCalendar> {
  return request(`/runs/${run}/jobs/${job}/calendar`);
}

export function publishCalendar(run: string, job: string, dates: Record<string, string>): Promise<PossessionCalendar> {
  return request(`/runs/${run}/jobs/${job}/calendar/publish`, {

...
def test_csv_export_rechecks_witness_after_green_report(client, solved, db_session):
    _, _, path = solved
    export_path = path.removesuffix("/calendar") + "/export"
...
def test_csv_export_rejects_failed_job_with_old_green_report(client, solved, db_session):
    _, job, path = solved
    db_session.get(ScenarioJob, uuid.UUID(job)).state = JobState.FAILED
    db_session.commit()
    assert client.get(path.removesuffix("/calendar") + "/export").status_code == 409

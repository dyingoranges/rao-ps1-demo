...
def get_export(db: Session, run_id, job_id) -> tuple[str, bytes]:
    ...
    # CSV and calendar share the current persisted-witness assurance gate.
    from app.modules.calendar.service import assured

    job, _, _, _, _ = assured(db, run_id, job_id)
